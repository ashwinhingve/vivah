# Drizzle migration-journal drift — 0030–0041 are untracked in file-side meta

> **Status (updated 2026-07-29):** file-side meta drift **still open** — journal +
> snapshots stop at 0029, so fix this **before the next `drizzle-kit generate`**,
> as an operator-supervised task. The **DB-side ledger drift is RESOLVED on prod**:
> apply-state of 0030–0041 was verified live on 2026-07-29 (all objects present;
> idempotent re-run of 0031–0041 no-oped, except 0038's designed `city_id`
> backfill linking 12 vendors) and `drizzle.__drizzle_migrations` was seeded
> through 0041 via `scripts/db/reconcile-ledger-0030-0041-2026-07-29.sql`
> (42 rows, high-water 0041). Originally recorded 2026-07-17 by the Sprint A
> Phase 0 schema-freeze verification, when the drift covered only 0030/0031.

## The facts (verified 2026-07-29)

Twelve committed migrations (`0030` … `0041`) were applied as **raw hand-written
SQL that bypassed `drizzle-kit generate`**, so drizzle's file-side meta
(journal + snapshots) knows nothing about them:

| Range | Objects (summary) | In `_journal.json`? | Snapshots `meta/00NN_snapshot.json`? | In prod DB ledger? | Objects present in prod DB? |
|---|---|---|---|---|---|
| `0030` | pgvector embedding → 768 dim + HNSW index | ❌ no | ❌ absent | ✅ seeded 2026-07-29 | ✅ verified live |
| `0031`–`0040` | support tickets · financial shells · virtual dates/retention · NRI cols · scale indexes · destination weddings · phase-8 supply · marketing/cities · city links · referral credits | ❌ no | ❌ absent | ✅ seeded 2026-07-29 | ✅ verified live |
| `0041` | 3 `audit_event_type` enum values (forward-only) | ❌ no | ❌ absent | ✅ seeded 2026-07-29 | ✅ verified live |

- `_journal.json` last entry: **idx 29 / `0029_pgvector_embedding`**.
- `meta/` snapshots: **`0000_snapshot.json` … `0029_snapshot.json`** (stop at 0029).
- `drizzle.__drizzle_migrations` (**prod**): **42 rows** (0000–0041) as of
  2026-07-29. Ledger hashes = sha256 of the raw file bytes; `created_at` for
  0030–0041 = git add-commit millis (absent from the journal). Local docker DB
  ledger may still be at 30 rows — run the same reconcile script there before any
  local `drizzle migrate`.
- Committed `.sql` files run **0000 → 0041** (+ `rollback-0030…`, `rollback-0031…`).
...
(schema frozen at 0040 per the 2026-07-26 operator directive; 0041 is enum-only).
...
1. The next `generate` will **re-emit** every object added by 0030–0041 (they're
   all absent from the 0029 snapshot), and
...
3. **Regenerate the reconciling migration** (Windows PowerShell preferred per
   ...
   already-applied objects of 0030–0041, plus a fresh snapshot + journal entry.
   **Inspect the emitted SQL** — it must contain ONLY those already-existing
   objects (all `CREATE TABLE IF NOT EXISTS` / additive `ALTER`), nothing
   destructive.
4. **Resolve the 0030 numbering collision** in the same PR. Cleanest option: the
   single generated file becomes the canonical drizzle record of the 0030–0041
   objects — **remove** the twelve hand-written raw files and their `rollback-*`
   counterparts, preserving their content in the PR description for provenance.
   If that path is taken, the prod ledger rows seeded by
   `reconcile-ledger-0030-0041-2026-07-29.sql` must be replaced by the generated
   file's own hash row (delete the 12 seeded rows, insert the new one).
   (Alternative: keep the raw files and manually renumber the generated file +
   its journal idx to `0042` — messier; only if history must be preserved verbatim.)
