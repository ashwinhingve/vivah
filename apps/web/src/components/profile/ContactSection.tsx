...
export function ContactSection({ phone, email, isSelf }: Props) {
  ...
  if (!hasContact && !isSelf) {
    ...
    return (
      <div className="rounded-xl bg-[#FEFAF6] border border-[#E8E0D8] p-4 flex items-center gap-3">
        {/* Lock icon */}
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[#C5A47E]/15 flex items-center justify-center">
        ...
        <div>
          <p className="text-sm font-medium text-[#2E2E38]">Contact Details</p>
          <p className="text-xs text-[#6B6B76] italic mt-0.5">Visible after mutual interest</p>
        </div>
      </div>
    );
  }
  ...
  // Unlocked state
  return (
    <div className="rounded-xl bg-white border border-[#E8E0D8] p-4 space-y-3">
      <h3 className="text-sm font-semibold text-[#2E2E38]">Contact Details</h3>
      {phone && (
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-[#2E2E38]">{phone}</span>
        </div>
      )}
      {email && (
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-[#2E2E38]">{email}</span>
        </div>
      )}
    </div>
  );
