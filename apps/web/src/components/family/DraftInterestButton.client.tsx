...
export function DraftInterestButton({ childUserId, targetProfileId, candidateName }: Props) {
  ...
  if (done) {
    return (
      <div className="flex items-center justify-center gap-1.5 rounded-lg border border-success/30 bg-success/5 px-3 py-2 text-xs font-medium text-success">
        <CheckCircle2 className="h-4 w-4" /> Drafted — awaiting their approval
      </div>
    );
  }
  ...
  return (
    <div>
      <button
        ...
        onClick={() => {
          ...
          startTransition(async () => {
            ...
            if (!r.ok) setError(r.error ?? 'Could not draft the interest.');
            else setDone(true);
          });
        }}
      ...
      >
        <Heart className="h-4 w-4" /> {pending ? 'Drafting…' : 'Draft interest'}
      </button>
