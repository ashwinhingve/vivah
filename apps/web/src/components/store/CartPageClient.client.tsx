  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-muted-foreground">
        ...
        <Link
          href="/store"
          className="mt-2 px-6 py-2.5 min-h-[44px] flex items-center bg-teal text-white font-semibold rounded-lg text-sm hover:bg-teal/90 transition-colors"
        >
          Browse Products
        </Link>
      </div>
    ...
    <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-background">
              {item.imageKey ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <ShoppingBag className="h-8 w-8 text-gold/30" />
                </div>
              )}
            </div>
      ...
      <div className="bg-surface border border-gold/20 rounded-xl p-4 h-fit sticky top-24">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="border-t border-gold/20 pt-2 flex justify-between font-semibold text-foreground">
            <span>Total</span>
            <span className="text-teal">₹{subtotal.toLocaleString('en-IN')}</span>
          </div>
        </div>

        <button
          onClick={() => router.push('/store/checkout')}
          className="w-full mt-4 min-h-[44px] bg-teal text-white font-semibold rounded-lg text-sm hover:bg-teal/90 transition-colors"
        >
          Proceed to Checkout
        </button>
