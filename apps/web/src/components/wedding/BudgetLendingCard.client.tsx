...
import { recordBudgetLendingConsentAction } from '@/app/[locale]/(app)/weddings/[id]/budget/actions';

interface BudgetLendingCardProps {
  weddingId: string;
...
export function BudgetLendingCard({
  weddingId,
  shortfallAmount,
  initialOffers,
  isPreview,
}: BudgetLendingCardProps) {
  const t = useTranslations('budgetLending');
  const [selected, setSelected] = useState<string | null>(null);
  const [consent, setConsent] = useState(false); // NEVER pre-ticked — RBI compliance
  ...
  const handleContinue = async () => {
    ...
    setStatus('saving');
    setError(null);
    const result = await recordBudgetLendingConsentAction({
      weddingId,
      offerRef: selected,
    });
    if (result.success) {
  ...
  return (
    <div className="space-y-6 rounded-2xl border border-gold/20 bg-surface p-4 sm:p-6 shadow-card">
      {/* Section header */}
      <div>
        ...
        <p className="mt-1 text-sm text-text-muted">
          {t('description', { shortfall: `₹${shortfallAmount.toLocaleString('en-IN')}` })}
        </p>
      </div>
      ...
      {/* LSP disclosure — Smart Shaadi is not the lender (RBI compliance) */}
      ...
      {status === 'done' ? (
        ...
        <>
          {/* Neutral multi-offer list — no steering, no highlighted "best" (RBI compliance) */}
          <RadioGroup
            className="gap-4"
            value={selected ?? ''}
          ...
          {/* Explicit, un-pre-ticked consent (RBI compliance) */}
          <div className="rounded-2xl border border-gold/20 bg-surface p-4 shadow-card">
            <label className="flex items-start gap-3 text-sm text-foreground min-h-[44px]">
              <input
                ...
                data-testid="lending-consent-checkbox"
              />
              <span>
                {t('consentText')}
              </span>
