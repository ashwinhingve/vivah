      ...
      {initialDocs.length === 0 ? (
        ...
        <div className="bg-white border border-[#C5A47E]/20 rounded-xl shadow-sm overflow-x-auto">
          <table className="w-full text-sm min-w-[640px]">
            ...
            <tbody>
              {initialDocs.map(d => (
                <tr key={d.id} className="border-b border-[#C5A47E]/10 last:border-0">
                  <td className="px-4 py-3 font-medium">
                    {d.url ? (
                      <a href={d.url} target="_blank" rel="noreferrer" className="text-[#7B2D42] hover:underline">{d.label}</a>
                    ) : d.label}
                  </td>
