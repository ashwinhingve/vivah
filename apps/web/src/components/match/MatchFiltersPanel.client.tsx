...
/**
 * MatchFiltersPanel — the heavy filter UI (Accordion + Slider + chip toggles).
 *
 * Split out from MatchFilters.client.tsx so it can be `next/dynamic`-imported
 * with `ssr: false` by consumers (DesktopFilterSidebar + MatchFeed mobile
 * sheet). Lightweight shared exports (types, DEFAULT_FILTERS, ActiveFilterChips,
 * option constants) stay in MatchFilters.client.tsx so consumers keep them
 * eager — only this panel module is deferred.
 *
 * IMPORTANT: all filtering is client-side over already-loaded feed items
 * (see comment in MatchFilters.client.tsx — no feed-filter API exists).
 */
