...
export function UpgradeCTA(props: BlurProps | InlineProps) {
  ...
  if (props.variant === 'inline') {
    return (
      <div className="rounded-xl border border-amber-200 bg-gradient-to-br from-amber-50 to-yellow-50 p-5 shadow-sm">
    ...
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-white/40 backdrop-blur-[2px]">
        <div className="rounded-2xl border border-slate-200 bg-white px-5 py-4 text-center shadow-lg">
          ...
          <h3 className="mt-2 text-sm font-semibold text-slate-900">{heading}</h3>
          <p className="mt-1 text-xs text-slate-600">{sub}</p>
          <Link
            href="/settings/billing"
