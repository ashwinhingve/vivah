...
export function IncidentsClient({ weddingId, initial }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {open ? (
        <form onSubmit={submit} className="space-y-3 rounded-2xl border border-foreground/10 bg-surface p-4 shadow-card">
          <div>
      ...
      {incidents.length > 0 ? (
        <ul className="divide-y divide-foreground/5 rounded-2xl border border-foreground/10 bg-surface shadow-card">
          {incidents.map((i) => (
            <li key={i.id} className="flex items-start justify-between gap-3 p-4">
              ...
              {!i.resolvedAt ? (
                ...
                <span className="shrink-0 rounded-full bg-success/15 px-2 py-1 text-2xs font-medium uppercase tracking-wide text-success">Resolved</span>
              )}
            </li>
          ))}
        </ul>
