    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-4 py-8 pb-24">
        ...
        {!analytics || analytics.totalGuests === 0 ? (
          <div className="bg-surface border border-dashed border-gold/30 rounded-xl p-10 text-center mt-6">
              ...
              <Card title="By meal preference">
                <div className="space-y-2 text-sm">
                  {(Object.keys(analytics.byMealPref) as MealPref[]).map(m => (
                    <Bar key={m} label={MEAL_LABEL[m]} value={analytics.byMealPref[m]} max={analytics.totalGuests} color="var(--color-teal)" />
                  ))}
                </div>
              </Card>
              ...
              <Card title="By side">
                <div className="space-y-2 text-sm">
                  {(['BRIDE', 'GROOM', 'BOTH', 'UNKNOWN'] as const).map(s => (
                    <Bar key={s} label={s === 'UNKNOWN' ? 'Unspecified' : s} value={analytics.bySide[s]} max={analytics.totalGuests} color="var(--color-primary)" />
                  ))}
                </div>
              </Card>

              <Card title="Check-in (day-of)">
                <div className="text-3xl font-semibold text-teal">{analytics.checkedIn}<span className="text-base text-muted-foreground"> / {analytics.totalGuests}</span></div>
                    ...
                    return (
                      <div key={t.date} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-3 bg-secondary rounded overflow-hidden relative">
                          <div className="absolute inset-y-0 left-0 bg-gold/40" style={{ width: `${(t.sent / total) * 100}%` }} />
                          <div className="absolute inset-y-0 left-0 bg-teal/70" style={{ width: `${(t.responded / total) * 100}%` }} />
                        </div>
...
function Bar({ label, value, max, color = 'var(--color-primary)' }: { label: string; value: number; max: number; color?: string }) {
