  ...
  return (
    <div className="space-y-5">
      <form ref={formRef} action={formAction} className="space-y-4 rounded-xl border border-gold/20 bg-surface p-4 shadow-card sm:p-6">
        <h3 className="font-heading text-base text-primary">Block a date</h3>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="date" className="mb-1 block text-sm font-medium text-primary">Date</label>
            <input id="date" name="date" type="date" required disabled={pending} className={inputCls} />
          </div>
          <div>
            <label htmlFor="reason" className="mb-1 block text-sm font-medium text-primary">Reason (optional)</label>
            <input id="reason" name="reason" maxLength={255} disabled={pending} className={inputCls} placeholder="Already booked, holiday…" />
          </div>
        </div>
        ...
        <button type="submit" disabled={pending}
          ...
          <CalendarPlus className="h-4 w-4" /> {pending ? 'Blocking…' : 'Block date'}
        </button>
      </form>

      {initial.length > 0 && (
        <div>
          <p className="mb-2 text-sm font-medium text-primary">Blocked dates</p>
          <ul className="space-y-2">
            {initial.map((d) => (
              <li key={d.id} className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-2.5 shadow-card">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    startRemove(async () => {
                      ...
                      if (!r.ok) setRemoveErr(r.error ?? 'Could not remove that date.');
                    });
                  }}
