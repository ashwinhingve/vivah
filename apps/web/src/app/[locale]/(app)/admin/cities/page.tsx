...
import { CityNetwork } from './CityNetwork.client';
import type { CityNetworkOverview } from '@smartshaadi/types';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'adminCities.metadata' });
  return { title: t('title') };
}

export default async function CitiesAdminPage() {
  const t = await getTranslations('adminCities');
  const me = await fetchAuth<AuthMe>('/api/auth/me');
  if (me && me.role !== 'ADMIN') {
    return await redirect(me.role === 'SUPPORT' ? '/support' : '/dashboard');
  }

  const overview = await fetchAuth<CityNetworkOverview>('/api/v1/admin/cities');
