...
import { MarketingOverview } from './MarketingOverview.client';
...
export default async function MarketingAdminPage() {
  ...
  const [stats, campaigns] = await Promise.all([
    fetchAuth<MarketingOverviewStats>('/api/v1/admin/marketing/overview'),
    fetchAuth<{ items: MarketingCampaign[]; total: number }>('/api/v1/admin/marketing?limit=50'),
  ]);
  ...
  return (
    <PageTransition className="min-h-full bg-background px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6">
          <MarketingOverview stats={stats ?? undefined} campaigns={campaigns?.items ?? []} />
        </div>
      </div>
    </PageTransition>
  );
