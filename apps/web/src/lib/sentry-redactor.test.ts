...
describe('redactSentryEvent', () => {
  ...
  it('should not redact phone numbers starting with 0-5', () => {
    const event = {
      message: 'Phone: 5555555555 is fine, but 9876543210 is redacted',
    };
    const result = redactSentryEvent(event);
    expect(result.message).toContain('5555555555');
  ...
  it('should redact phone in breadcrumb data', () => {
    const event = {
      breadcrumbs: [
        {
    ...
    expect(bcData?.phone).toBe('[redacted-phone]');
    expect(bcData?.name).toBe('John');
  });
  ...
  it('should redact in request query string', () => {
    const event = {
      request: {
    ...
    expect(req.query_string).toBe('phone=[redacted-phone]&email=[redacted-email]');
  });
  ...
  it('should handle arrays in extra', () => {
    const event = {
      extra: {
        items: [
    ...
    const result = redactSentryEvent(event);
    const extra = result.extra as Record<string, unknown>;
    const items = extra.items as string[];
    expect(items[0]).toBe('Contact: [redacted-phone]');
    expect(items[1]).toBe('Email: [redacted-email]');
    expect(items[2]).toBe('Name: John');
  });
});
