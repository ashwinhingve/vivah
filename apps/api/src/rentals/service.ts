...
function toRentalItem(row: RentalItemRow): RentalItem {
  return {
    id:          row.id,
...
export async function listRentalItems(query: RentalListQuery): Promise<{
  ...
  // Availability filter: if both dates provided, exclude fully-booked items
  let filteredRows = rows as RentalItemRow[];

  if (fromDate && toDate) {
    // For each item, sum overlapping active booking quantities
    // Overlap condition: NOT (booking.toDate < fromDate OR booking.fromDate > toDate)
    //   = booking.fromDate <= toDate AND booking.toDate >= fromDate
    ...
    if (itemIds.length > 0) {
      ...
      filteredRows = filteredRows.filter((item) => {
        ...
        return reserved < item.stockQty;
      });
    }
  }

  return {
    items: filteredRows.map(toRentalItem),
...
export async function getRentalItem(itemId: string): Promise<RentalItem> {
  ...
  return toRentalItem(rows[0] as RentalItemRow);
}
...
export async function createRentalItem(
  ...
  return toRentalItem(inserted[0] as RentalItemRow);
}
...
export async function createRentalBooking(
  ...
  if (reserved + input.quantity > item.stockQty) {
    throw makeError('CONFLICT', 'Insufficient stock for the requested dates', 409);
  }
  ...
  const inserted = await db
    ...
    .values({
      ...
      customerId:   userId,   // TEXT — Better Auth user.id passes through directly
  ...
  const row = inserted[0] as RentalBookingRow;
