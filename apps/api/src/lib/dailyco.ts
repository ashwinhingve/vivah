...
import { dailycoBreaker } from './circuit-breaker.js';
...
export async function createRoom(
  ...
  return dailycoBreaker.call(async () => {
...
export async function getRoom(roomName: string): Promise<DailyRoom | null> {
  ...
  try {
    return await dailycoBreaker.call(async () => {
      const res = await fetch(`https://api.daily.co/v1/rooms/${roomName}`, {
        headers: { Authorization: `Bearer ${env.DAILY_CO_API_KEY}` },
      });
    ...
    // Circuit breaker open or other error — return null (graceful degradation)
    return null;
  }
}
